...
export default function HomePage({
  ...
  return (
    <div className="flex flex-col h-full bg-white w-full max-w-[393px] mx-auto">
      {/* Header */}
        ...
        <div className="space-y-3">
          ...
          <div className="relative">
            <div className="flex items-center bg-white rounded-lg overflow-hidden">
              ...
              <button
                ...
                className="px-4 py-3 bg-[#365a19] hover:bg-[#2d4a15] transition-colors flex-shrink-0"
              >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-2 bg-white bg-opacity-20 rounded-lg px-3 py-2 hover:bg-opacity-30 transition-all flex-shrink-0"
            >
            ...
            {showLocationFilter && (
              <input
                ...
                className="flex-1 px-3 py-2 rounded-lg text-black text-sm outline-none min-w-0"
                onKeyPress={(e) => e.key === 'Enter' && handleSearch()}
              />
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {categories.map((category) => (
              <button
                ...
                className="bg-gray-50 border border-gray-200 rounded-xl p-3 hover:bg-gray-100 transition-colors min-h-[90px] flex flex-col justify-center"
              >
                <div className="text-center">
                  <div className="text-2xl mb-2">{category.icon}</div>
                  <h3 className="font-medium text-sm leading-tight">{category.name}</h3>
                  <p className="text-xs text-gray-600 mt-1 leading-tight">{category.description}</p>
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="p-4 pb-8">
          <div className="bg-gradient-to-r from-[#426b1f] to-[#365a19] rounded-xl p-4 text-white">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                onClick={onViewAllProducts}
                className="bg-white bg-opacity-20 rounded-lg py-3 px-3 hover:bg-opacity-30 transition-all"
              >
                <div className="text-center">
...
function ProductCard({
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-md transition-shadow flex flex-col h-[240px] w-full">
      <div className="relative flex-shrink-0">
        ...
        <div className="absolute top-2 left-2 flex flex-col gap-1">
          ...
          {showUrgentBadge && (
            <span className="bg-red-500 text-white text-[9px] px-1.5 py-0.5 rounded-full font-medium">
              🚨 Urgent
            </span>
          )}
        </div>
        ...
        >
          <span className={`text-xs ${product.isFavorite ? 'text-red-500' : 'text-gray-400'}`}>
      ...
      <div className="p-3 flex flex-col flex-1 justify-between">
        <div className="flex-1">
          <h3 className="font-medium text-xs mb-1 line-clamp-2 min-h-[2rem] leading-tight">{product.name}</h3>
        ...
        <button
          onClick={onClick}
          className="w-full bg-[#426b1f] text-white py-2 rounded-lg hover:bg-[#365a19] transition-colors text-xs font-medium mt-auto"
        >
          View Product
        </button>
      </div>
